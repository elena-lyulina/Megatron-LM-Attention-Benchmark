...
# One job per (suffix, model) covers every requested (offset, prefix) point, sharing one
# checkpoint load. Points already done in store are dropped before submitting; a group with
# nothing left submits no job. To add a model to the sweep: add it to llama_checkpoints.sh.
#
...
for SUFFIX in "${SUFFIXES[@]}"; do
    for MODEL in "${MODELS[@]}"; do
        ...
        # Points still needing work for this (suffix, model) -- calls Stage 2's own --dry-run
        # (dual-checks scratch then store, rep-aware -- catches a pkl left stale by a
        # REPETITIONS increase, not just "file exists") instead of a separate coarse checker.
        if [[ $FORCE -eq 1 || $FORCE_METRICS -eq 1 ]]; then
            GROUP_POINTS=("${PAIRS[@]}")
        else
            NEEDED=$(python3 "$SCRIPT_DIR/../evaluation/compute_memorization_metrics.py" \
                --dry-run \
                ...
                --points "${PAIRS[@]}" --suffix-length "$SUFFIX")
            IFS=' ' read -r -a GROUP_POINTS <<< "$NEEDED"
        fi
        SKIPPED_COUNT=$((SKIPPED_COUNT + ${#PAIRS[@]} - ${#GROUP_POINTS[@]}))

        if [[ ${#GROUP_POINTS[@]} -eq 0 ]]; then
            echo "All ${#PAIRS[@]} point(s) already complete for (model=$MODEL suffix=$SUFFIX) -- nothing to submit."
            continue
        fi
        ...
        echo "Submitting measure_mem.slurm (model=$MODEL exp=$EXP_NAME backend=$BACKEND) points=$POINTS_CSV suffix_length=$SUFFIX (${#GROUP_POINTS[@]} points, 1 job)"
