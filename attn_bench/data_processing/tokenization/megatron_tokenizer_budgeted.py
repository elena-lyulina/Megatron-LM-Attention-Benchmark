"""
Copied from https://github.com/swiss-ai/data-pipeline-pretrain/blob/yxu/support-local-tokenizer-and-parquet/src/data_pipeline_pretrain/pipeline/tokens/megatron_tokenizer.py
Added budgeting support + explicit EOS handling
"""
...
class MegatronDocumentTokenizer(PipelineStepWithTokenizer):
    ...
    Args:
        ...
        tokenizer_name_or_path (str): the name or path of the tokenizer to use, from the HuggingFace tokenizers library (default: None)
        add_bos (bool): whether to add BOS token from the tokenizer before each document (default: True)
        add_eos (bool): whether to add EOS token from the tokenizer after each document (default: True)
    ...
    def __init__(
        ...
        self.upload_block_size = upload_block_size
        self.add_special_tokens = add_bos or add_eos

    @property
    def tokenizer(self):
        """Load tokenizer using AutoTokenizer to support local paths.

        Overrides parent's tokenizer property to use transformers.AutoTokenizer
        which supports both HuggingFace Hub IDs and local file paths, unlike
        tokenizers.Tokenizer.from_pretrained() which only supports Hub IDs.
        """
        if self._tokenizer is None:
            ...
            is_local_path = os.path.isabs(self.tokenizer_name_or_path)
            ...
            if not hf_tokenizer.is_fast:
                raise ValueError(f"Tokenizer at {self.tokenizer_name_or_path} is not a fast tokenizer")

            self._tokenizer = hf_tokenizer._tokenizer
            ...
            if self.add_bos:
                ...
                if bos_token is None:
                    raise ValueError(
                        f"add_bos=True but tokenizer at {self.tokenizer_name_or_path} has no bos_token. "
                        f"Pass add_bos=False to skip."
                    )
            ...
            if self.add_eos:
                ...
                if eos_token is None:
                    raise ValueError(
                        f"add_eos=True but tokenizer at {self.tokenizer_name_or_path} has no eos_token. "
                        f"Pass add_eos=False to skip."
                    )
    ...
    def write_tokens(self, data: DocumentsPipeline, filename: str) -> MegatronTokenizedFile:
...
class BudgetedMegatronDocumentTokenizer(MegatronDocumentTokenizer):
    """MegatronDocumentTokenizer that stops each worker after a token budget is reached.
