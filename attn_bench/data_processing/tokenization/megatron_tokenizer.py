...
class MegatronDocumentTokenizer(PipelineStepWithTokenizer):
    ...
    Args:
        ...
        add_special_tokens (bool): whether to add special tokens (BOS/EOS) during tokenization (default: True)
            Set to False if your data already has BOS/EOS tokens from apply_chat_template to avoid double BOS/EOS.
    """
    ...
    def __init__(
        ...
        add_special_tokens: bool = True,  # Whether to add special tokens (BOS/EOS) during tokenization
    ):
        super().__init__()
    ...
    def tokenizer(self):
        ...
        if self._tokenizer is None:
            ...
            if is_local_path:
                # For local absolute paths, use local_files_only to avoid Hub validation
                hf_tokenizer = AutoTokenizer.from_pretrained(
                    self.tokenizer_name_or_path,
                    local_files_only=True
                )
            else:
                # For Hub IDs (e.g., "gpt2", "meta-llama/Llama-2-7b"), load from Hub
                hf_tokenizer = AutoTokenizer.from_pretrained(self.tokenizer_name_or_path)
