# KDA inference tests: verify KimiDeltaAttention greedy generation is correct.
#   - oracle_selfcheck     : the cacheless quadratic oracle == one teacher-forced forward.
#   - decode_matches_oracle: the cached/incremental _decode path == the quadratic oracle, token for
#                            token. Tests KimiDeltaAttention._decode + the prefill state cache
#                            (key_value_memory_dict), i.e. fused_recurrent_kda + causal_conv1d_update.
# Mirrors attn_bench/tests/test_gdn_inference.py. See attn_bench/_plans/kda_packed_and_inference_plan.md.
...
def greedy_cached(model, prompt_ids, suffix_length, return_logits=False):
    # The path under test: prefill the prompt into a StaticInferenceContext (writes the KDA conv +
...
def _make_test_oracle_selfcheck(base_forward_step):
    ...
    def test_oracle_selfcheck(model):
        ...
        if not any(isinstance(m, KimiDeltaAttention) for _, m in model.named_modules()):
            print_rank_0("[FAIL] oracle_selfcheck: no KimiDeltaAttention module found in model")
            return False
...
def _make_test_decode_matches_oracle(base_forward_step):
    ...
    def test_decode_matches_oracle(model):
        ...
        if not any(isinstance(m, KimiDeltaAttention) for _, m in model.named_modules()):
            print_rank_0("[FAIL] decode_matches_oracle: no KimiDeltaAttention module found in model")
            return False
...
def register(base_forward_step):
    # called by registry.py to resolve test functions for the 'kda_inference' suite;
