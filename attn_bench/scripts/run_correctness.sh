...
##### Paths #####
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)" # BASH_SOURCE[0] has the path of the current file, we just cd a few dirs up and get the repo abs path dynamically (works from everywhere)
RESULTS_DIR="${REPO_ROOT}/results/correctness"
...
TRAINING_CMD="torchrun ${TORCHRUN_ARGS[*]} -m attn_bench.benchmarks.correctness \
    ${BENCHMARK_ARGS[*]} \
    ${TRANSFORMER_ENGINE_ARGS[*]} \
    ${DISTRIBUTED_ARGS[*]} \
    ${NETWORK_SIZE_ARGS[*]} \
